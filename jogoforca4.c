...
char* escolher_palavra(char grupo) {
    ...
    const char* animais[] = {"cachorro", "gato", "elefante", "tigre", "leao"};
    const char* filmes[] = {"matrix", "titanic", "avatar", "rocky", "jumanji"};
    const char* objetos[] = {"mesa", "cadeira", "computador", "telefone", "laptop"};
...
void jogar_forca(char* palavra) {
    ...
    while (tentativas > 0 && !acertou) {
        ...
        if (letra_ja_escolhida(letra, letras_usadas, usadas_contador)) {
            printf("Você já escolheu essa letra! Tente outra.\n");
...
void iniciar_jogo() {
    ...
    do {
        ...
        printf("Escolha a opção: Jogar sozinho (1), Jogar acompanhado (2): ");
        scanf("%d", &opcao);
